  ...
  renderField() {
    ...
    for (let i = 0; i < this.newGame.getRows(); i++) {
      for (let j = 0; j < this.newGame.getColumns(); j++) {
        let cubeX =
          gameOptions.boardOffset.x + gameOptions.tileSize * j + gameOptions.tileSize / 2;
        let cubeY =
          gameOptions.boardOffset.y + gameOptions.tileSize * i + gameOptions.tileSize / 2;
    ...
    movements.forEach(
      function (movement) {
        ...
        this.tweens.add({
          ...
          y:
            this.newGame.getCustomDataAt(movement.row, movement.column).y +
            gameOptions.tileSize * movement.deltaRow,
    ...
    replenishMovements.forEach(
      function (movement) {
        ...
        sprite.y =
          ...
          gameOptions.tileSize * (movement.row - movement.deltaRow + 1) -
          gameOptions.tileSize / 2;
        (sprite.x =
          gameOptions.boardOffset.x +
          gameOptions.tileSize * movement.column +
          gameOptions.tileSize / 2),
        ...
        this.tweens.add({
          ...
          y:
            gameOptions.boardOffset.y +
            gameOptions.tileSize * movement.row +
            gameOptions.tileSize / 2,
          duration: gameOptions.fallSpeed * movement.deltaRow,
          callbackScope: this,
          onComplete: function () {
