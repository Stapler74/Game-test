export default class GameGen {
  ...
  validPick(row, column) {
    return (
      ...
      column >= 0 &&
      column < this.columns &&
      this.newGameArray[row] !== undefined &&
      this.newGameArray[row][column] !== undefined
    );
  }
  ...
  floodFill(row, column) {
    ...
    if (
      this.newGameArray[row][column].value === this.colorToLookFor &&
      !this.alreadyVisited(row, column)
    ) {
